fun main() {
    ...
    data class State(val pawns: List<Pawn>, val rolls: Int, val isFinal: Boolean)
    ...
    fun State.next(): State? = when (this.isFinal) {
        true -> null
        false -> {
            ...
            State(pawns.drop(1) + newPawn, rolls + 3, newPawn.score >= 1000)
        }
    }
    ...
    fun Map<State, Long>.next(): Map<State, Long>? {
        if (keys.all { it.isFinal }) {
            return null
        }
        ...
        return entries
            .flatMap { (state, count) ->
                when (state.isFinal) {
                    true -> listOf(state to count)
                    false -> {
                        val pawn = state.pawns.first()
                        diracRollTotals.map { (rollTotal, rollCount) ->
                            val newPosition = ((pawn.position + rollTotal - 1) % 10) + 1
                            val newPawn = pawn.copy(position = newPosition, score = pawn.score + newPosition)
                            val newState = State(state.pawns.drop(1) + newPawn, state.rolls + 3, newPawn.score >= 21)
            ...
            .aggregate { _, sum, (_, count), _ -> (sum ?: 0L) + count }
    }

    fun part1(input: List<String>): Int {
        val initialState = parseInput(input)
        val finalState = generateSequence(initialState) { state -> state.next() }.last()
        return finalState.pawns.minOf { it.score } * finalState.rolls
    }

    fun part2(input: List<String>): Long {
        val initialMap = mapOf(parseInput(input) to 1L)
        val finalMap = generateSequence(initialMap) { it.next() }.last()
        ...
        return winnerIdCounts.maxOf { it.value }
    }
